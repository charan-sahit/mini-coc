...
class Hut(Building):
    ...
    def render(self,colorArray):
        ...
        """ else:
            self.color = Back.YELLOW + "1" + Style.RESET_ALL """
    ...
    def idUpdate(self,idArray):
        ...
        if(self.death == True):
            idArray[self.coordinates[0]][self.coordinates[1]] = 0
        return idArray
...
class Canon(Building):
    ...
    def shoot(self,king,Barbarians,Archers):
        if(self.death==False):
            min_distance = 10
            #print(self.occupied)
            self.target = None
...
class Townhall(Building):
    ...
    def render(self,colorArray,idArray):
        if(self.health>0):        
            ...
            for i in range(self.rows):
                for j in range(self.columns):
                    colorArray[int(40/2)-1+i][int(80/2)-1+j]= Back.LIGHTGREEN_EX + " " + Style.RESET_ALL
                    idArray[int(40/2)-1+i][int(80/2)-1+j]= 0
