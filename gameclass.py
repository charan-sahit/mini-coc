...
class Game:
    ...
    def board(self,rows,columns):
        ...
        #self.colorArray = [[self.color for i in range(self.columns)] for j in range(self.rows)]
        self.colorArray = [[Back.LIGHTGREEN_EX + " " + Style.RESET_ALL for i in range(columns)] for j in range(rows)]
        self.idArray = [[0 for i in range(columns)] for j in range(rows)]
        """ for i in range(3):
            for j in range(4):
